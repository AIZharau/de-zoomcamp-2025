...
- `select * from dtc_zoomcamp_2025.raw_nyc_tripdata.green_taxi`
    ...
    Taking into account errors in environment names (DBT_BIGQUERY_SOURCE_DATASET != DBT_BIGQUERY_DATASET)
...
**Explanation**:
1. var("days_back", ...):
- This is a dbt-variable, which can be passed via the command line 
 (dbt run --vars '{"days_back": 7}').

- If the days_back variable is not set on the command line, dbt will use the default value specified by the second argument.

2. env_var("DAYS_BACK", ...):

- This is an environment variable that can be set in the system 
(export DAYS_BACK=7).

- If the DAYS_BACK is not set, the default value specified by the second argument will be used.

3. Default value:

- If neither days_back nor DAYS_BACK are specified, the default value of 30 will be used.
    **ANSWER**:
- Update the WHERE clause to `pickup_datetime >= CURRENT_DATE - INTERVAL '{{ var("days_back", env_var("DAYS_BACK", "30")) }}' DAY`
