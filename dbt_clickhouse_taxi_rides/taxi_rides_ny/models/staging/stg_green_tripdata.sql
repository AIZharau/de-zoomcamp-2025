...
SELECT
    ...
    CAST(
        CASE 
            WHEN ehail_fee = '' OR ehail_fee IS NULL THEN '0'  -- Handling empty lines and NULL
            ELSE ehail_fee
        END AS Float64
    ) AS ehail_fee,
    CAST(
        CASE 
            WHEN improvement_surcharge = '' OR improvement_surcharge IS NULL THEN '0'  -- Handling empty lines and NULL
